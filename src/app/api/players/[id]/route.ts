...
export async function GET(
	...
	const runs = rows
		.filter((r) => r.eventId)
		.map((r) => {
			const splitCount = r.splitCount ?? DEFAULT_SPLIT_COUNT;
			const amountOwed =
				r.amount ?? (r.totalCost != null ? r.totalCost / splitCount : 0);
			const amountPaid: number | null =
				r.amountPaid ?? (r.paid ? amountOwed : null);
			return {
				eventId: r.eventId,
...
export async function PATCH(
	...
	queries.updatePlayerProfile.run({
		userId: params.id,
		displayName:
			body.displayName !== undefined ? body.displayName : existing.displayName,
