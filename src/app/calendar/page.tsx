...
export default function CalendarPage() {
	...
	const firstLabel = new Date(year, month, 1).toLocaleDateString("en-CA", {
		month: "short",
		year: "numeric",
	});
	const last = displayMonths[2];
	const lastLabel = new Date(last.year, last.month, 1).toLocaleDateString(
		"en-CA",
		...
		<div className="space-y-4">
			...
			<div className="sm:hidden space-y-5">
				{displayMonths.map((dm) => {
					const label = new Date(dm.year, dm.month, 1).toLocaleDateString(
						"en-CA",
					...
					return (
						<div key={`${dm.year}-${dm.month}`} className="space-y-2">
							...
							{monthRuns.length === 0 ? (
								...
								monthRuns.map((run) => {
									const runDate = new Date(run.startDate).toLocaleDateString(
										"en-CA",
										{
											weekday: "short",
											month: "short",
						...
						<div key={`${dm.year}-${dm.month}`} className="space-y-1">
							...
							<div className="rounded-lg border overflow-hidden">
								...
								{weeks.map((week, wi) => (
									<div
										key={wi}
										className={`grid grid-cols-7 ${wi > 0 ? "border-t" : ""}`}
									>
										{week.map((day, di) => {
											...
											return (
												...
												>
													<span
														className={`inline-flex items-center justify-center text-xs w-6 h-6 rounded-full font-medium
